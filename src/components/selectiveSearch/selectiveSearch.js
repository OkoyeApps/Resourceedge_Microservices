                        ...
                        <ul className="w-100">
                            {
                                result.length === 0 ?
                                    <li>Searching...</li>
                                    :
                                    result && result.slice(0, 10).map((item, index) => (<li onClick={(e) => handleSelect({ email: item.email, EmployeeId: item.employeeId, name: item.fullName }, e)}
                                        key={index} className="w-100 d-flex align-items-center"><ReactAvatar className="rounded-circle mr-2" size={'20'} name={item.fullName} /> {item.fullName}</li>))
                            }
                        </ul>
                    </div> : <></>
                    : <></>
